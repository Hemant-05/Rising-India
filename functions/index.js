...
// Payment status change -> notify user + admins (includes refund)
...
console.log(`Payment ${paymentId} status: ${prev} → ${curr}`);

try {
  // Notify user (if token exists)
  if (after.userId) {
    const userDoc = await db.collection('users').doc(after.userId).get();
    const token = userDoc.exists ? (userDoc.data() || {}).fcmToken : null;

    if (token) {
      const amount = toRupees(after.amount);
      let title = '';
      let body = '';
      let type = '';

      if (curr === 'success' || curr === 'completed') {
        title = '✅ Payment Successful';
        body = `Payment of ₹${amount} processed successfully.`;
        type = 'payment_success';
      } else if (curr === 'failed' || curr === 'error') {
        title = '❌ Payment Failed';
        body = `Payment of ₹${amount} could not be processed. Please try again.`;
        type = 'payment_failed';
      } else if (curr === 'refund' || curr === 'refunded') {
        title = '↩️ Payment Refunded';
        body = `Refund of ₹${amount} has been initiated for your order.`;
        type = 'payment_refund';
      }
      ...
      if (title) {
        await queueNotification({
          ...
          userId: after.userId,
          source: 'payment_status_user',
        });
        console.log('✅ User payment notification queued');
      }
    }
  }

  // Always notify admins on payment status update
  const amount = toRupees(after.amount);
  await notifyAdminsBatch(() => ({
    title: `Payment ${curr === 'success' || curr === 'completed' ? 'Received' : curr === 'refund' || curr === 'refunded' ? 'Refunded' : 'Update'}`,
    body: `Payment ${curr} for order #${shortId(after.orderId || '')} - ₹${amount}`,
    data: {
      paymentId,
      orderId: after.orderId || '',
    ...
    source: 'payment_status_admin',
  }));

  console.log('✅ Admin payment notifications queued');
} catch (error) {
  console.error('❌ Error in payment status change handler:', error);
}
return null;
});*/
